...
def carregar_local_evento():
    if os.path.exists('local_evento.json'):
        with open('local_evento.json', 'r', encoding='utf-8') as file:
...
def carregar_servicos_bebidas():
    if os.path.exists('servicos_bebidas.json'):
        with open('servicos_bebidas.json', 'r', encoding='utf-8') as file:
            return json.load(file)
    else:
        return {"bebidas": []}
...
def carregar_servicos_buffet():
    if os.path.exists('servicos_buffet.json'):
        with open('servicos_buffet.json', 'r', encoding='utf-8') as file:
            return json.load(file)
    else:
        return {"buffet": []}
...
def salvar_banda_artista(artista):
    with open('artista.json', 'w') as file:
        json.dump(artista, file, indent=4)

def carregar_servicos_seguranca():
    if os.path.exists('seg.json'):
        with open('seg.json', 'r', encoding='utf-8') as file:
            return json.load(file)
    else:
        return {"seguranca": []}

def salvar_servicos_seguranca(seguranca):
   with open('seg.json', 'w') as file:
        json.dump(seguranca, file, indent=4)


def carregar_servicos_fotografia():
    if os.path.exists('fotografia.json'):
        with open('fotografia.json', 'r', encoding='utf-8') as file:
            return json.load(file)
    else:
        return {"fotografia": []}

def salvar_servicos_fotografia(fotografia):
    with open('fotografia.json', 'w') as file:
        json.dump(fotografia, file, indent=4)
...
def escolhas():
    ...
    match(codigo):
        ...
        case 1:
            ...
            print("Novo local adicionado com sucesso!")
            voltamenu()
        ...
        case 5:
            ...
            seg = {}
            seg["nome_servico_seguranca"] = input('Nome do Serviço de Segurança: ')
            seg['localizacao']={}
            seg["localizacao"]["cidade"] = input('Localização (Cidade): ')
            seg["localizacao"]["estado_provincia"] = input('Localização (Estado/Província): ')
            seg["telefone_contato"] = input('Telefone de Contato: ')
            seg["email_contato"] = input('E-mail de Contato: ')
            seg["especialidades_seguranca"] = input('Especialidades de Segurança: ')
            seg['equipe_seguranca']={}
            seg["equipe_seguranca"]["numero_funcionarios"] = input('Equipe de Segurança (número de funcionários): ')
            seg["equipe_seguranca"]["experiencia"] = input('Equipe de Segurança (experiência): ')
            seg["licencas_certificacoes"] = input('Licenças e Certificações (se aplicável): ')
            seg['disponibilidade_servico']={}
            seg["disponibilidade_servico"]["horarios_disponiveis"] = input('Horários Disponíveis: ')
            seg["disponibilidade_servico"]["dias_semana"] = input('Dias da Semana (separados por vírgula): ').split(',')
            seg['faixa_preco_servico']={}
            seg["faixa_preco_servico"]["preco_base"] = input('Preço base por Serviço: ')
            seg["faixa_preco_servico"]["custos_adicionais"] = input('Custos adicionais (se aplicável): ')

            seguranca['seguranca'].append(seg)
            salvar_servicos_seguranca(seguranca)
            voltamenu()
        ...
        case 6:
            ...
            servicos_fotografia["localizacao"] = {}
            ...
            servicos_fotografia["disponibilidade_agendamento"] = {}
            ...
            servicos_fotografia["faixa_preco_servico"] = {}
            ...
            servicos_fotografia["redes_sociais_website"] = {}
            ...
            salvar_servicos_fotografia(fotografia)
            voltamenu()
        ...
        case 8:
            ...
            limpar()
            print("Escolha o serviço que deseja excluir:")
            print("1 - Local para evento")
            print("2 - Serviço de Bebidas")
            print("3 - Serviço de Buffet")
            print("4 - Bandas e Artistas")
        ...
        case 9:
            ...
            limpar()
            print("Escolha o serviço que deseja visualizar:")
            print("1 - Local para evento")
            print("2 - Serviço de Bebidas")
            print("3 - Serviço de Buffet")
            print("4 - Bandas e Artistas")
            ...
            if visualizar == 1:
                ...
                limpar()
                if os.path.exists('local_evento.json'):
                    with open('local_evento.json', 'r', encoding='utf-8') as loc:
                        data = json.load(loc)
                    
                    if "locais" in data and len(data["locais"]) > 0:
                ...
                limpar()
                if os.path.exists('servicos_bebidas.json'):
                    with open('servicos_bebidas.json', 'r', encoding='utf-8') as loc:
                        data = json.load(loc)
                    
                    if "bebidas" in data and len(data["bebidas"]) > 0:
                        ...
                        for beb in data['bebidas']:
                            ...
                            print (f'Tipos de bebida oferecidas: {beb['tipos_bebidas_oferecidas']}')
                ...
                limpar()
                if os.path.exists('servicos_buffet.json'):
                    with open('servicos_buffet.json', 'r', encoding='utf-8') as loc:
                        data = json.load(loc)
                    
                    if "buffet" in data and len(data["buffet"]) > 0:
                        print('Serviços Cadastrados: ')
                        for buf in data['buffet']:
                            ...
                            print(f'Tipo de eventos atendidos: {buf['tipos_eventos_atendidos']}')
                ...
                limpar()
                if os.path.exists('artista.json'):
                    with open('artista.json', 'r', encoding='utf-8') as loc:
                        data = json.load(loc)
                    
                    if "artista" in data and len(data["artista"]) > 0:
                        ...
                        for art in data['artista']:
                            ...
                            print(f'Horários disponíveis: {art['disponibilidade_agendamento']['horarios_disponiveis']} ')
                        voltamenu()
                    else:
                ...
                limpar()
                if os.path.exists('seg.json'):
                    with open('seg.json', 'r', encoding='utf-8') as loc:
                        data = json.load(loc)
                    
                    if "seguranca" in data and len(data["seguranca"]) > 0:
                        ...
                        for seg in data['seguranca']:
                            ...
                            print(f'Especialidade de segurança: {seg['especialidades_seguranca']}')
                            ...
                            print(f'Horários disponíveis: {seg['disponibilidade_servico']['horarios_disponiveis']} ')
                        voltamenu()
                    else:
                        print('Não há serviços de segurança cadastrados!')
                        voltamenu()

            elif visualizar == 6:
                limpar()
                if os.path.exists('fotografia.json'):
                    with open('fotografia.json', 'r', encoding='utf-8') as loc:
                        data = json.load(loc)
                    
                    if "fotografia" in data and len(data["fotografia"]) > 0:
                        ...
                        for fot in data['fotografia']:
                            ...
                            print('Disponibilidade de serviço: ')
                            print(f'Dias da semana disponíveis: {fot['disponibilidade_agendamento']['dias_semana']} ')
                            print(f'Horários disponíveis: {fot['disponibilidade_agendamento']['horarios_disponiveis']} ')
                            print ('Redes Sociais:')
                            print (f'Instagram: {fot['redes_sociais_website']['instagram']}')
                            print (f'Website: {fot['redes_sociais_website']['website']}')
                        voltamenu()
                    else:
                        print ('Não há serviços de fotografia cadastrados!')
                        voltamenu()            
            else:
                print ('Código Inválido!')
                voltamenu()
